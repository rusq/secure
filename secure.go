...
func DeriveKey(pass []byte, keySz int) ([]byte, error) {
	...
	if keySz <= 0 || keySz%8 != 0 {
		return nil, ErrInvalidKeySz
	}
	iterations := DeriveIter
	if iterations <= 0 {
		return nil, errors.New("invalid key derivation iteration count")
	}
	configMu.RLock()
	keySalt := append([]byte(nil), salt...)
	configMu.RUnlock()

	return pbkdf2.Key(pass, keySalt, iterations, keySz, sha512.New), nil
}
...
func armor(packed []byte) string {
	configMu.RLock()
	sig, encoding := signature, b64encoding
	configMu.RUnlock()
	return sig + encoding.EncodeToString(packed)
}
...
func unarmor(s string) ([]byte, error) {
	...
	if len(s) < sigSz || s[0:sigSz] != sig {
		return nil, ErrNotEncrypted
	}
	packed, err := encoding.DecodeString(s[sigSz:])
	if err != nil {
		return nil, err
	}
	return packed, nil
